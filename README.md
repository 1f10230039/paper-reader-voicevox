...
このアプリは VOICEVOX ENGINE が返す**すべての話者**を選べる。話者ごとに権利者と利用規約が異なり、
公式サイトの利用規約ページにまとまっている。音源利用規約はクレジット表記（例：`VOICEVOX:ずんだもん`）を
求めており、アプリでの利用では紹介画面などへの記載が必要とされているため、
**設定パネルに、選んでいる話者に合わせた `VOICEVOX:話者名` を表示している**。
作成した音声を動画などで公開する場合も、その話者の規約に従い、同じ形式のクレジット表記が必要になる。
