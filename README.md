...
上のバーは再生・前後の文・速度、右端は最前面の小窓と設定。左上の小さな表示が今の音声で、
VOICEVOX につながると話者名（ずんだもん など）になる（下の画像はブラウザ内蔵の音声で撮ったもの）。
...
このアプリは VOICEVOX ENGINE が返す**すべての話者**を選べる。話者ごとに権利者と利用規約が異なる。
各話者の規約は公式サイトの利用規約ページと、
[VOICEVOX/voicevox_resource](https://github.com/VOICEVOX/voicevox_resource) の `character_info/*/policy.md` にまとまっている。

どの話者も「`VOICEVOX:話者名` とクレジットを記載すれば利用可能」という形で、
アプリでの利用では紹介画面などへの記載が求められているため、
**設定パネルに、選んでいる話者に合わせた `VOICEVOX:話者名` を表示している**。
ただし、作成した音声を**商用**で使う場合や**企業として**使う場合には、話者によって事前確認や
問い合わせが必要なものがある（例：No.7 と ユーレイちゃん は非商用のみ、もち子さん は音声作品・ゲーム作品を除く、
Voidoll・ぞん子・後鬼・青山龍星 は法人・企業利用に個別確認）。
作成した音声を公開する場合は、選んだ話者の規約を確認すること。
